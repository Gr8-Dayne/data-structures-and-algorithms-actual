...
const returnNames = (arr) => {
  return arr.reduce((soup, currVal) => {
    soup.push(currVal.name);
    return soup;
  }, []);
};
...
const reversedString = (str) => {
  let array = str.split('');
  return array.reduce((sauce, val, idx, array) => {
    if (array.length - idx - 2 >= 0) {
      sauce += array[array.length - idx - 2];
    }
    return sauce;
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((juice, val) => {
    if (val.children) {
      juice += val.children.length;
    }
    return juice;
  }, 0);
};
...
const calculateAverage = (arr) => {
  return arr.reduce( (drip, val) => (drip + val) ) / arr.length;
};
