...
// addNumbers(8, [], 5, addValues).length.toStrictEqual(5);
...
const removeWithAnon = (arr) => {
  arr.forEach( (thing, whatever, arr) => {
    removeOne(thing, arr);
  });
  return arr
};
...
const createList = (availableItems) => {
  let groceryList = [];
  availableItems.forEach( (stuff) =>{
    if(stuff.available === true){
      groceryList.push(stuff.name);
    }
  })
  return groceryList;
};
...
const fizzbuzz = (arr) => {
  let bigArray = [];
  arr.forEach((stuffAgain) => {
    if(stuffAgain % 15 === 0){
      bigArray.push('Fizz Buzz');
    }else if(stuffAgain % 3 === 0){
      bigArray.push('Fizz');
    }else if(stuffAgain % 5 === 0){
      bigArray.push('Buzz');
    }else{
      bigArray.push(stuffAgain);
    }
  })
  return bigArray;
};
