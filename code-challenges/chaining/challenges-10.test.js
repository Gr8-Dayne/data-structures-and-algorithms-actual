...
const divisibleByFiveTwoToThePower = (input) => {
  let storagePlace = [];
  let dBFTTP = input.filter( (val, idx, arr) => {
    let inArray = arr[idx].filter( (val2, idx2, arr2) => {
      if (Number.isInteger(val2) && val2 % 5 === 0) {
        return val2;
      }
    });
    let outArray = inArray.map( val => Math.pow(2, val));
    storagePlace.push(outArray);
    return outArray;
  });
  return storagePlace;
};
...
let findMaleAndFemale = function (data){
  let count = 0;
  let str = '';
  data.forEach(elem => {
...
let findShortest = function (data){
  let shortest = data[0];
