...
const filterStringsWithVowels = (arr) => {
  let vowelSort = /[aeiouAEIOU]/g;
  return arr.filter( value =>
    value.match(vowelSort)
  )
};
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter( value => {
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter(value => {
...
const getStatName = (arr, minBaseStat) => {
  let statData = arr.filter(value => {
    return value.baseStat > minBaseStat;
  });
  return statData.map(elem => elem.stat.name)
};
...
const getCharactersWithoutChildren = (arr) => {
  let hahKids = arr.filter(value => {
    if (!value.children){
      return value;
    }
  });
  return hahKids.map(elem => elem)
}
